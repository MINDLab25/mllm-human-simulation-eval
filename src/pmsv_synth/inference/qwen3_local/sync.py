"""Sequential local inference for PMSV prediction using Qwen3-Omni-30B-A3B-GPTQ-4bit.

The model is loaded once as a singleton. All participant-video pairs are
processed sequentially on a single GPU — no ThreadPoolExecutor. Both zero-shot
and chain-of-thought (--cot) modes are supported.
...
def _normalize_keys(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    "arous arous" instead of "arousing").  We use difflib to snap each key to
    the nearest canonical name.
    """
    import difflib
...
def run_sync(
    ...
    **kwargs,
    ...
    """Run local Qwen3-Omni-30B-A3B-GPTQ-4bit inference sequentially.
    ...
    **kwargs        : ignored (kept for API compatibility with other providers)
    ...
    if n_skipped:
        print(
            f"[sync/qwen3-local] {n_skipped} pairs already done (resume) — "
    ...
    print(
        f"[sync/qwen3-local] Processing {len(pending)} participant-video pairs "
    ...
    try:
        for row in tqdm(pending, desc="Pairs (local Qwen3)"):
                ...
                try:
                    ...
                    if attempt < _MAX_RETRIES:
                        print(
                            ...
                            f"attempt {attempt}/{_MAX_RETRIES} failed ({type(exc).__name__}: {exc})\n{tb}",
                            file=sys.stderr,
                        )
                    else:
                        print(
                            f"\n[warn] Skipping participant {pid} / video {vid} "
                            f"after {_MAX_RETRIES} attempts: {type(exc).__name__}: {exc}\n{tb}",
                            file=sys.stderr,
                        )
